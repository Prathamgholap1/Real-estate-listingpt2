...
if(isset($_POST['post'])){
   ...
   $Ran = $_POST['Ran'];
   $Ran = filter_var($Ran, FILTER_SANITIZE_STRING);

  // Validate Rera Authentication Number format first
if (!preg_match('/^[A-Za-z]{2}[0-9]{8}$/', $Ran)) {
   $warning_msg[] = 'Invalid format. RERA Number must start with 2 letters followed by 8 digits.';
} else {
   // Check if the RERA number already exists
   $check_ran = $conn->prepare("SELECT * FROM `rera_numbers` WHERE `rera_number` = ?");
   $check_ran->execute([$Ran]);

   // If it doesn't exist, insert it into the rera_numbers table
   if($check_ran->rowCount() == 0){
      $insert_ran = $conn->prepare("INSERT INTO `rera_numbers`(`rera_number`) VALUES(?)");
      $insert_ran->execute([$Ran]);
   }

   // Now proceed to property upload
   if($image_01_size > 2000000){
      $warning_msg[] = 'Image 01 size too large!';
   } else {
      $insert_property = $conn->prepare("INSERT INTO `property`(id, user_id, property_name, address, price, type, offer, status, furnished, bhk, deposite, bedroom, bathroom, balcony, carpet, age, total_floors, loan, lift, security_guard, play_ground, garden, water_supply, power_backup, parking_area, gym, shopping_mall, hospital, school, market_area, image_01, image_02, image_03, image_04, image_05, description) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"); 
      $insert_property->execute([$id, $user_id, $property_name, $address, $price, $type, $offer, $status, $furnished, $bhk, $deposite, $bedroom, $bathroom, $balcony, $carpet, $age, $total_floors, $loan, $lift, $security_guard, $play_ground, $garden, $water_supply, $power_backup, $parking_area, $gym, $shopping_mall, $hospital, $school, $market_area, $rename_image_01, $rename_image_02, $rename_image_03, $rename_image_04, $rename_image_05, $description]);
...
<section class="property-form">
   ...
   <form action="" method="POST" enctype="multipart/form-data">
      ...
      <div class="box">
         <p>Rera Authentication Number <span>*</span></p>
         <input type="text" name="Ran" required maxlength="10" placeholder="Rera Authentication Number" class="input">
      </div>
