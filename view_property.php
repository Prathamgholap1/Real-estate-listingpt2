     ...
     <div class="swiper images-container">
      ...
      <div class="flex">
         ...
         <div class="box">
            ...
            <p><i>room floor :</i><span><?= $fetch_property['room_floors']; ?></span></p>
...
<h3 class="title">RERA Number</h3>
<?php
   // Get RERA number from the property
   $rera_number = $fetch_property['rera_numbers'] ?? $fetch_property['Ran'] ?? '';

   if (!empty($rera_number)) {
      $get_rera = $conn->prepare("SELECT * FROM `rera_numbers` WHERE `rera_number` = ?");
      $get_rera->execute([$rera_number]);

      if ($get_rera->rowCount() > 0) {
         $rera_data = $get_rera->fetch(PDO::FETCH_ASSOC);
         echo '<p class="description">' . htmlspecialchars($rera_data['rera_number']) . '</p>';
      } else {
         echo '<p class="description">RERA number not found in the system.</p>';
      }
   } else {
      echo '<p class="description">No RERA number assigned to this property.</p>';
   }
?>
